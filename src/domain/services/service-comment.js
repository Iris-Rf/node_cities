...
const ormComment = require('../orm/orm-comment');
...
exports.GetAll = async (req, res) => {
  ...
  let response = {};
  try {
    let respOrm = await ormComment.GetAll();
    if (respOrm.err) {
      (status = 'Failure'),
      ...
      (message = 'Success GetAll comments'),
        (data = respOrm),
...
exports.Create = async (req, res) => {
  ...
  try {
    ...
    if (title && text && author) {
      let respOrm = await ormComment.Create(title, text, author);
      if (respOrm.err) {
        (status = 'Failure'),
          (errorcode = respOrm.err.code),
